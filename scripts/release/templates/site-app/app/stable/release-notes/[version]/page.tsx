...
export default async function StableReleaseNotesVersionPage({ params }: VersionPageProps) {
  const { version } = await params;
  const normalizedVersion = version.replace(/\.html$/i, "");
  const latest = await readLatestRelease("stable");
  const notes = await readReleaseNotes("stable", normalizedVersion);

  if (!notes) {
    notFound();
  }

  const versionLabel = formatReleaseVersionLabel("stable", notes.version, notes.build);
  const latestSlug = extractReleaseNotesSlug(latest?.releaseNotesUrl);
  const publishedAt = latestSlug === notes.slug ? formatPublishedAt(latest?.publishedAt) : null;
